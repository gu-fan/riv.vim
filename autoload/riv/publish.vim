...
fun! riv#publish#repl_file_link(line) "{{{
    ...
    " ./index.rst  =>  `./index  <./index.html>`_
    " ../          =>  `../      <../index.html>`_
    " index.rst#RIV => `index#RIV <index.html#RIV>`_     ??
    ...
    while !empty(file)
        ...
        if !s:is_relative(file)
            let line = substitute(line, s:escape_file_ptn(file), s:gen_embed_link(file, file), 'g')
        elseif s:is_directory(file)
            let title = file
            let path = title .index . '.html'
            let line = substitute(line, s:escape_file_ptn(file), s:gen_embed_link(title, path), 'g')
        else
            if file =~ '\.'.rst_ext.'$'
...
fun! s:create_tmp(file) "{{{
    update
    let lines = map(readfile(a:file),'riv#publish#repl_file_link(v:val)')
    " let lines = map(getline(1,'$'),'riv#publish#repl_file_link(v:val)')
